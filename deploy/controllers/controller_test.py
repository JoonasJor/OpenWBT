...
class Runner:
    ...
    def post_squat(self):
        if self.locoable():
            self.transfer_to_loco = keyboard.run_loco_signal
    ...
    def post_loco(self):
        if self.stopable():
            if keyboard.stopgait_signal:
                self.loco_controller.stance_command = True
            self.transfer_to_squat = keyboard.run_squat_signal
...
class Runner_online_real(Runner_online):
    ...
    def damping_state(self):
        ...
        while not keyboard.start_signal:
            create_damping_cmd(self.low_cmd)
    ...
    def run_squat(self, debug=True, manual=True):
        ...
        if manual:
            ...
            cmd_raw[0] = keyboard.move_x  # height
            cmd_raw[1] = keyboard.turn_x
        else:
            cmd_raw = None
